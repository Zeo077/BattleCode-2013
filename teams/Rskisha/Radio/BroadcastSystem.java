...
//				e.printStackTrace();
	...
	public static void write(EncampmentJob e)
	{
		write(e.channel, e.buildMessage());
	}
	...
	public static int[] getChannelNos(ChannelType channelType, int constant) {
		...
		for (int i = 0; i < Constants.REDUNDANT_CHANNELS; i++) {
			...
			int offset = ((Integer.toString(((constant << 4 + Constants.CHANNEL_CYCLE * channelType.ordinal()) << 4 + i)).hashCode())+rc.getTeam().ordinal()) % ChannelType.range;
	...
	public static void clear(ChannelType chan)
	{
		if(rc!=null)
		{
			try {
				for (int channelNo : getChannelNos(chan)) 
				{
					rc.broadcast(channelNo, 0);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
